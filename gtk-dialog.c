...
vrfy_fingerprint_data_new(otrg_plugin_fingerprint *fprint);
...
static void smp_progress_response_cb(GtkDialog *dialog, gint response,
  ...
  PurpleConversation *conv =
      otrg_plugin_conversation_to_purple_conv(context, 0);
...
static int start_or_continue_smp(SmpResponsePair *smppair) {
  ...
  if (!smppair->responder) {
    ...
    otrg_plugin_continue_smp(smppair->conv, (const unsigned char *)secret,
                             secret_len);
  }
...
static void add_to_vbox_init_one_way_auth(GtkWidget *vbox,
  ...
  otrg_plugin_fingerprint *fp =
      otrg_plugin_fingerprint_get_active(smppair->conv->peer);
...
static void add_to_vbox_init_two_way_auth(GtkWidget *vbox,
  ...
  otrg_plugin_fingerprint *fp =
      otrg_plugin_fingerprint_get_active(auth_opt_data->smppair->conv->peer);
  if (fp && fp->trusted)
    label2 = gtk_label_new(_("This buddy is already authenticated."));
...
static void add_to_vbox_verify_fingerprint(GtkWidget *vbox,
  ...
  otrg_plugin_fingerprint *fprint =
      otrg_plugin_fingerprint_get_active(username);
...
static GtkWidget *create_smp_dialog(const char *title, const char *primary,
                                    const otrg_plugin_conversation *pconv,
...
static void
otrg_gtk_dialog_notify_message(PurpleNotifyMsgType type,
...
otrg_gtk_dialog_private_key_wait_start(const char *account,
                                       const char *protocol) {
  PurplePlugin *p;
...
static void
otrg_gtk_dialog_unknown_fingerprint(OtrlUserState us, const char *accountname,
...
conn_context_to_plugin_conversation(ConnContext *context) {
  ...
  otrg_plugin_conversation *plugin_conv =
      malloc(sizeof(otrg_plugin_conversation));
  if (!plugin_conv) {
    return NULL;
  }
...
static void vrfy_fingerprint_changed(GtkComboBox *combo, void *data) {
  ...
  // otrg_plugin_fingerprint_get(vfd->fprint->fingerprint)
  fprint = vfd->fprint;
...
static void verify_fingerprint(GtkWindow *parent,
  ...
  secondary = g_strdup_printf(
      ...
      fprint->account, fprint->protocol, our_hash, fprint->username,
      fprint->fp);
...
static void
otrg_gtk_dialog_verify_fingerprint(otrg_plugin_fingerprint *fprint) {
...
static void
otrg_gtk_dialog_socialist_millionaires(const otrg_plugin_conversation *conv,
                                       const char *question,
                                       gboolean responder) {
  char *primary;
...
static void otrg_gtk_dialog_update_smp(const otrg_plugin_conversation *context,
  ...
  if (progress_level == 0.0) {
    ...
    if (smp_event == OTRNG_SMP_EVENT_SUCCESS) {
      ...
      otrg_plugin_fingerprint *fp =
          otrg_plugin_fingerprint_get_active(context->peer);
      if (fp && !responder) {
        fp->trusted = 1;
        otrg_plugin_write_fingerprints();
        otrg_ui_update_keylist();
...
static void
otrg_gtk_dialog_connected_real(const otrg_plugin_conversation *context) {
  PurpleConversation *conv;
  char *buf;
  char *format_buf;
  TrustLevel level;
  ...
  if (*is_multi_inst) {
    ...
    if (!*have_warned_instances) {
      *have_warned_instances = TRUE;
      ...
      otrg_gtk_dialog_display_otr_message(context->account, context->protocol,
                                          context->peer, buf, 0);
      g_free(buf);
    }
  }
...
static void
otrg_gtk_dialog_disconnected_real(const otrg_plugin_conversation *context) {
  PurpleConversation *conv;
  char *buf;
  OtrgUiPrefs prefs;
...
static GList *otr_get_full_buddy_list(PurpleConversation *conv) {
  ...
  if (gtkconv->active_conv->type == PURPLE_CONV_TYPE_IM) {
    ...
    if (buds == NULL && !g_list_find(conv_list, conv)) { /* buddy not on list */
      conv_list = g_list_prepend(conv_list, conv);
    } else {
      for (l = buds; l != NULL; l = l->next) {
        PurpleBlistNode *node =
            (PurpleBlistNode *)purple_buddy_get_contact((PurpleBuddy *)l->data);
            ...
            if (!g_list_find(pres_list, presence)) {
              ...
              PurpleConversation *currentConv =
                  purple_find_conversation_with_account(
                      PURPLE_CONV_TYPE_IM, purple_buddy_get_name(buddy),
                      purple_buddy_get_account(buddy));

              pres_list = g_list_prepend(pres_list, presence);

              if (currentConv != NULL && !g_list_find(conv_list, currentConv)) {
...
static void unselect_meta_ctx(PurpleConversation *conv) {
  GtkWidget *select_best =
      (GtkWidget *)purple_conversation_get_data(conv, "otr-select_best");
  GtkWidget *select_recent =
      (GtkWidget *)purple_conversation_get_data(conv, "otr-select_recent");
...
static void select_meta_ctx(GtkWidget *widget, gpointer data) {
  ...
  otrl_instag_t *selected_instance =
      (otrl_instag_t *)purple_conversation_get_data(conv,
                                                    "otr-ui_selected_ctx");
  ...
  if (widget == select_best) {
    ...
    if (value) {
      ...
      context = (ConnContext *)otrg_plugin_conv_to_selected_context(conv, 1);

      recent_context = (ConnContext *)otrg_plugin_conv_to_context(
          conv, OTRL_INSTAG_RECENT_RECEIVED, 0);
      if (context != recent_context) {
        gchar *buf = g_strdup_printf(
            _("Warning: The selected outgoing "
...
static void build_meta_instance_submenu(PurpleConversation *conv,
  ...
  GtkWidget *select_best =
      gtk_check_menu_item_new_with_label(_("Send to most secure"));
  GtkWidget *select_recent =
      gtk_check_menu_item_new_with_label(_("Send to most recent"));
  otrl_instag_t *selected_instance =
...
static void
otr_add_buddy_instances_top_menu(PidginConversation *gtkconv, GList *instances,
  ...
  for (; instances; instances = instances->next) {
    ...
    ConvOrContext *curr_convctx =
        g_hash_table_lookup(conv_or_ctx_map, curr_context);
    gboolean selected =
        (curr_context->their_instance == context->their_instance);
    gint instance_i = -1;
    ...
    if (!selection_exists ||
      ...
      gtk_menu_shell_prepend(GTK_MENU_SHELL(instance_submenu), select_ctx);
      gtk_widget_show(select_ctx);
    } else if (selection_exists &&
               *selected_instance == curr_context->their_instance) {
      GtkWidget *selected_ctx = gtk_menu_item_new_with_label(_("Selected"));
      GtkWidget *menusep = gtk_separator_menu_item_new();
      ...
      gtk_menu_shell_prepend(GTK_MENU_SHELL(instance_submenu), selected_ctx);
 ...
 */
static void
otr_add_buddy_top_menu(PidginConversation *gtkconv,
                       const otrg_plugin_conversation *conv,
                       const ConvOrContext *convctx, // TODO: Remove convctx
                       gboolean active_conv, const char *username,
                       const char *accountname, int *pos) {
  PidginWindow *win = pidgin_conv_get_window(gtkconv);
  GtkWidget *menu_bar = win->menu.menubar;
...
static void otr_add_buddy_top_menus(PurpleConversation *conv) {
  ...
  for (list_iter = g_list_last(full_buddy_list); list_iter != NULL;
    ...
    int num_contexts = 1; // TODO: Identify when there's more than one
    ...
    is_multi_instance =
        purple_conversation_get_data(currentConv, "otr-conv_multi_instances");
    ...
    if (num_contexts > 1) {
      ...
      *is_multi_instance = TRUE;
      otr_add_buddy_instances_top_menu(gtkconv, NULL, active_conv,
                                       ...
                                       currentContext->accountname, &pos);
    } else {
      ...
      if (!convctx) {
        convctx = malloc(sizeof(ConvOrContext));
        g_hash_table_insert(conv_or_ctx_map, currentConv, (gpointer)convctx);
        convctx->convctx_type = convctx_conv;
      ...
      username = g_strdup(
          purple_normalize(account, purple_conversation_get_name(currentConv)));

      otrg_plugin_conversation *plugin_conv =
          purple_conversation_to_plugin_conversation(currentConv);
      otr_add_buddy_top_menu(gtkconv, plugin_conv, convctx, active_conv,
                             username, accountname, &pos);
      otrg_plugin_conversation_free(plugin_conv);
      g_free(username);
    }
  }
...
static gboolean check_incoming_instance_change(PurpleAccount *account,
  ...
  received_context = (ConnContext *)otrg_plugin_conv_to_context(
      conv, (otrl_instag_t)OTRL_INSTAG_RECENT_RECEIVED, 0);
  ...
  if (have_received &&
    ...
    dialog_update_label_conv(conv,
                             otrg_plugin_conversation_to_trust(plugin_conv));
    otrg_plugin_conversation_free(plugin_conv);
  }
...
static void otrg_gtk_dialog_init(void) {
  ...
  purple_signal_connect(purple_get_core(), "quitting", otrg_plugin_handle,
                        PURPLE_CALLBACK(dialog_quitting), NULL);
}

/* Deinitialize the OTR dialog subsystem */
static void otrg_gtk_dialog_cleanup(void) {
  purple_signal_disconnect(purple_get_core(), "quitting", otrg_plugin_handle,
