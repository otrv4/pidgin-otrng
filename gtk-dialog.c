...
static gchar *get_text_main() {
  ...
  text = g_strdup_printf(
      ...
      "OTRng-the plugin you are using-"
      "is the plugin that implements",
	  "the 4th version of the OTR protocol.",
      "This version provides better deniability properties by the use",
	  "of a deniable authenticated key exchange (DAKE),",
	  "and better forward secrecy through the use of the double",
	  "ratchet algorithm.");
...
static GtkWidget * otr_about_info(gchar *label){
...
static void set_notebook_tab(GtkWidget *notebook, char *tab_title,
                             gchar *tab_content_main, guint tab_index) {
...
static GtkWidget *get_notebook(gint page_num) {
  ...
  gchar *text_main, *text_properties, *text_cryptographic,
      *text_conversation_status, *text_credits;
  ...
  text_credits = otr_about_text_credits();

  set_notebook_tab(notebook, _("Main Information"), text_main, 0);
  set_notebook_tab(notebook, _("OTRv4 Properties"), text_properties, 1);
...
static void otr_show_help_dialog(gint page_num) {
  ...
  dialog = gtk_dialog_new_with_buttons(_("About OTRv4"), NULL, 0, NULL,
...
static void show_menu_authors_otrv4(GtkWidget *widget, gpointer data) {
  gint default_page = 4;
  otr_show_help_dialog(default_page);
}
...
static void otr_build_status_submenu(PidginWindow *win,
  ...
  understanding_otrv4 =
      gtk_image_menu_item_new_with_mnemonic(_("_About OTRv4"));
  ...
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), authors);
  ...
  gtk_signal_connect(GTK_OBJECT(authors), "activate",
                     GTK_SIGNAL_FUNC(show_menu_authors_otrv4), conv);
}
