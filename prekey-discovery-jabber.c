...
report_found_prekey_server(otrng_plugin_prekey_discovery_status *iq_status,
    ...
    unsigned char *bytefingerprint = hex_to_bytes(fingerprint,
...
static void
receive_prekey_connection_information(PurpleConnection *pc, const char *type,
	...
	if (purple_strequal(type, "result") &&
		...
		for (item = xmlnode_get_child(query, "item"); item;
            ...
            if(jid != NULL && purple_strequal(jid, from) &&
               node != NULL && purple_strequal(node, "fingerprint") &&
               fingerprint != NULL && strlen(fingerprint) == 112) {
...
static void
find_connection_information_for(PurpleConnection *pc, const char *jid,
...
static void
receive_server_info(PurpleConnection *pc, const char *type, const char *id,
...
static void
receive_server_items(PurpleConnection *pc, const char *type, const char *id,
                     const char *from, xmlnode *iq, gpointer data) {
	xmlnode *query;
...
void otrng_plugin_prekey_discovery_jabber_load() {
	iq_callbacks = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                         g_free);
}
